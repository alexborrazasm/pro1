/* Plantee e implemente un programa en Lenguaje C que reciba dos enteros 
positivos y clasifique como perfecto, abundante o deficiente cada número del 
intervalo comprendido entre ambos, sabiendo que un número deficiente es el que 
la suma de sus divisores propios (divisores positivos menores que el propio 
número) es menor que él; numero perfecto es el que la suma de sus divisores 
...
int clasificarNumero(int num) {
    // Inicializamos con 1, ya que 1 siempre es divisor propio.
    int sumaDivisores = 1; 
...
int main() {
    ...
    if (inicio >= fin || inicio <= 0 || fin <= 0) {
        printf("Error: Ingresa dos enteros positivos y asegúrate de que el "
